...
class UnetResidual(UnetBase):
    ...
    def bottleneck_block(self, x, filters, kernel_size=(3, 3), padding="same", strides=1):
        ...
        bottleneck = layers.Conv2D(filters, kernel_size=(1, 1), padding=padding, strides=strides)(x)
    ...
    def build(self, image_size, f):
        ...
        ## downsample  
        e1 = self.bottleneck_block(input_tensor, f); f = int(f*2)
        e2 = self.res_block(e1, f, strides=2); f = int(f*2)
        e3 = self.res_block(e2, f, strides=2); f = int(f*2)
        e4 = self.res_block(e3, f, strides=2); f = int(f*2)
        ...
        _ = self.res_block(_, f); f = int(f/2)
